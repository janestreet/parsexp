...
let gen_code oc (t : t) =
  ...
  in
  List.iter (ordered_ids named_transitions) ~f:(fun (id, tr) ->
    match tr with
    | Error error ->
    ...
    | Ok { action = eps_actions, action; goto; advance } ->
      ...
      pr
        ...
        (if Option.is_none action
         && not ([%compare.equal: goto_state] goto End_block_comment)
         then "_"
         else "");
      ...
      (match goto with
       ...
       | End_block_comment ->
         ...
         pr
           "  set_automaton_state state (if block_comment_depth state <> 0 then %d \
            else %d);"
           (State.to_int (Block_comment Normal))
           (State.to_int Whitespace));
      pr
        "  %s state;"
        (match advance with
         | Advance -> "advance"
  ...
  List.iteri State.all ~f:(fun i st ->
    pr "  %s %s" (if i = 0 then "[|" else " ;") (State.old_parser_approx_cont_state st));
  pr "  |]";
  pr "";
