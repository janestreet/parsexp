...
module Error = struct
  ...
  let raise : type a b. (a, b) state -> at_eof:bool -> Reason.t -> _ =
    ...
    let message =
      ...
      | Comment_token_in_unquoted_atom ->
        if String.equal (Buffer.contents state.atom_buffer) "|"
    ...
    let old_parser_exn =
      ...
      | Comment_token_in_unquoted_atom, _
        when String.equal (Buffer.contents state.atom_buffer) "|" -> `Failure
      | _ -> `Parse_error
    in
...
let check_new_sexp_allowed state =
  ...
  then Error.raise state ~at_eof:false Too_many_sexps
;;
...
let closing : type u s. (u, s) state -> char -> s -> s =
  ...
  else Error.raise state ~at_eof:false Closed_paren_without_opened
;;
