...
module type Parser = Parsexp_intf.Parser with module Error := Parse_error
...
struct
  let reraise positions parsed_value ~sub exn =
    ...
    raise (Of_sexp_error { user_exn = exn; sub_sexp = sub; location = loc })
  ;;
